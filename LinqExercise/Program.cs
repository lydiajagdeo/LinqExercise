...
namespace LinqExercise
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var printOnlyFour = numbers.OrderBy(x => x).Take(4);

            foreach (var item in printOnlyFour)
            {
            ...
            Console.WriteLine(filter.Sum(x=>x.YearsOfExperience));
            Console.WriteLine( filter.Average(x=> x.YearsOfExperience));

            //TODO: Add an employee to the end of the list without using employees.Add()

            employees.Append(new Employee("Lydia", "Jagdeo", 100, 75)).ToList().ForEach(x => Console.WriteLine($"Name:{x.FullName} Age:{x.Age} YOE:{x.YearsOfExperience} "));
